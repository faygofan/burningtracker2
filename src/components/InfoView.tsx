...
export const InfoView: React.FC<InfoViewProps> = ({ mapId, onBack }) => {
  ...
  // Ref for Update button to support "Reset -> Enter" workflow
  ...
  const handleReset = () => {
    ...
    setEditPlayers(0);
    setBurningError(null);
    // Move focus to Update button so Enter key triggers save
    setTimeout(() => updateBtnRef.current?.focus(), 0);
  };
      ...
      {isModalOpen && selectedChannel && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#2f3136] w-64 rounded-lg shadow-2xl border border-[#202225] overflow-hidden animate-in zoom-in-95 duration-200">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full bg-[#202225] text-[#facc15] py-2 px-2 rounded border outline-none text-center font-bold text-lg placeholder-neutral-600 ${burningError ? 'border-red-500 focus:border-red-500' : 'border-[#202225] focus:border-yellow-600'
                      }`}
                  />
                  {burningError && (
            ...
            <div className="bg-[#292b2f] px-3 py-3 flex justify-center gap-3">
              <button
                onClick={handleReset}
                className="flex-1 py-1.5 rounded text-sm font-medium transition-colors shadow-sm bg-red-600 hover:bg-red-700 text-white"
              >
                Reset
              </button>
              <button
                ...
                className={`flex-1 py-1.5 rounded text-sm font-medium transition-colors shadow-sm ${burningError
